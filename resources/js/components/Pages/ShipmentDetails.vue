<template>
    ...
    <div v-else-if="shipment" class="space-y-8 animate-fade-in-up">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-text-muted mt-1">Created on {{ new
                        Date(shipment.created_at).toLocaleDateString() }}</p>
                </div>
            </div>

            <div class="flex flex-wrap items-center gap-3 justify-end">

                <button @click="raiseTicket"
                    class="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 border border-red-100 text-sm font-bold rounded-lg hover:bg-red-100 transition-colors">
                    <span>🚑</span> Report Issue
                </button>

                <a v-if="getTrackingLink(shipment)" :href="getTrackingLink(shipment)" target="_blank"
                ...
                <span v-else
                    class="px-4 py-2 bg-gray-100 text-gray-400 text-sm rounded-lg cursor-not-allowed border border-gray-200"
                    title="No Tracking Link Available">
                    📍 Track (N/A)
                </span>

                <button v-if="shipment.label_url" @click="downloadLabel(shipment.label_url, shipment.tracking_number)"
                ...
                <div v-else-if="shipment.status === 'booked'" class="flex items-center gap-2">
                    <span
                        class="px-4 py-2 bg-amber-50 text-amber-600 text-sm font-medium rounded-lg cursor-wait border border-amber-100 flex items-center gap-2">
                        <span class="animate-pulse">⏳</span> Generating Label...
                    </span>
                    <button @click="fetchShipment" :disabled="loading"
        ...
        <div class="bg-surface rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div
                class="px-6 py-4 bg-slate-50 border-b border-slate-100 font-bold text-text-main flex items-center gap-2">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="item in shipment.items" :key="item.id">
                        ...
                        <td class="px-6 py-3 text-text-muted">{{ item.length }}x{{ item.width }}x{{ item.height }} cm
                        </td>
            ...
            <div v-if="shipment.quotes && shipment.quotes.length > 0" class="grid gap-4">
                <div v-for="quote in shipment.quotes" :key="quote.id"
                    ...
                    <div class="flex items-center gap-4 mb-4 md:mb-0 w-full md:w-auto">
                        <div
                            class="h-12 w-12 bg-slate-100 rounded-lg flex items-center justify-center text-xl shadow-inner">
                            🚚
                        </div>
                        <div>
                            ...
                            <div
                                class="text-xs text-green-600 font-bold mt-1 bg-green-50 inline-block px-2 py-0.5 rounded">
                                ETA: {{ quote.eta }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                        <div class="text-right">
                            <div class="text-xl font-bold text-text-main">${{ (quote.price_cents / 100).toFixed(2) }}
                            </div>
                            <div class="text-xs text-slate-400">ex GST</div>
                        </div>

                        <button v-if="shipment.status === 'draft'" @click="bookShipment(quote)"
...
const raiseTicket = () => {
    // Simply push to support page with query param
